...
WriteDataCallBack _WriteDataCallBk;
WriteDataArrayCallBack _WriteDataArrayCallBk;

uint8 _x, _page;

void _AddPosition(uint8 n)
{
    uint8 i = 0;
    while (i < n)
    {
...
#define _WriteData(_data)    \
    _WriteDataCallBk(_data); \
    _AddPosition(1)

#define _WriteDataArray(arr, len)    \
    _WriteDataArrayCallBk(arr, len); \
    _AddPosition(len)

void OLED_Init(WriteCmdCallBack writeCmdCallbk, WriteDataCallBack writeDataCallbk, WriteDataArrayCallBack writeDataArrayCallbk)
{
    _WriteCmd = writeCmdCallbk;
    _WriteDataCallBk = writeDataCallbk;
    _WriteDataArrayCallBk = writeDataArrayCallbk;
...
void OLED_SetColumn(uint32 col)
{
...
void OLED_SetPage(uint32 p)
{
    _WriteCmd(OLED_CALC_START_PAGE(p));
    _page = p;
}

uint8 OLED_GetColumn()
{
    return _x;
}
...
void OLED_SetColumnAndPage(uint32 col, uint32 p)
{
    OLED_SetColumn(col);
    OLED_SetPage(p);
}
...
{
    uint8 page = y / 8, pos = 0x01 << (y % 8), _data;
    OLED_SetColumnAndPage(x, page);
    _data = val ? (_data | pos) : (_data & ~pos);
    _WriteData(_data);
}

void OLED_WriteData_8x16(uint8 (*arr)[2][8])
{
    uint32 x = _x, page = _page;
    _WriteDataArray((*arr)[0], 8);
    OLED_SetColumnAndPage(x, page + 1);
    _WriteDataArray((*arr)[1], 8);
    OLED_SetPage(page);
}
...
{
    uint32 x = _x, page = _page;
    _WriteDataArray((*arr)[0], 16);
    OLED_SetColumnAndPage(x, page + 1);
    _WriteDataArray((*arr)[1], 16);
    OLED_SetPage(page);
}

void OLED_WriteDataArray(uint8 *_arr, uint8 _len)
{
    _WriteDataArray(_arr, _len);
}
...
void OLED_WriteString_8x16(const char *_str)
{
    ...
    {
        if (_str[index] < 32)
            OLED_WriteData_8x16(CHAR_8X16(32));
        else if (_str[index] > 126)
            OLED_WriteData_8x16(CHAR_8X16('?'));
        else
            OLED_WriteData_8x16(CHAR_8X16(_str[index]));
